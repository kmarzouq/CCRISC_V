...
Vsoc_top::Vsoc_top(VerilatedContext* _vcontextp__, const char* _vcname__)
    ...
    , tohost_wr{vlSymsp->TOP.tohost_wr}
    , uart_wr{vlSymsp->TOP.uart_wr}
    , uart_data{vlSymsp->TOP.uart_data}
    , tohost_data{vlSymsp->TOP.tohost_data}
    , rootp{&(vlSymsp->TOP)}
{
...
VL_ATTR_COLD void Vsoc_top::trace(VerilatedVcdC* tfp, int levels, int options) {
    if (tfp->isOpen()) {
        vl_fatal(__FILE__, __LINE__, __FILE__,"'Vsoc_top::trace()' shall not be called after 'VerilatedVcdC::open()'.");
    }
    if (false && levels && options) {}  // Prevent unused
    tfp->spTrace()->addModel(this);
    tfp->spTrace()->addInitCb(&trace_init, &(vlSymsp->TOP));
    Vsoc_top___024root__trace_register(&(vlSymsp->TOP), tfp->spTrace());
}
